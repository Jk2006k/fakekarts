...
export function updateHud(speed: number, peers: Iterable<Peer>) {
  ...
  document.querySelector('#players')!.innerHTML = `<span><i style="background:#ff5a4f"></i>YOU</span><span><i style="background:#30a9ff"></i>BOT-01</span>${remotePlayers.map(peer => `<span><i style="background:#a879ff"></i>${peer.name.replace(/[<>]/g, '')}</span>`).join('')}`
}
