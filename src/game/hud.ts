...
export function updateHud(speed: number, drift: number, health: number, botHealth: number, peers: Iterable<Peer>, localId: string, localScore: number) {
  ...
  document.querySelector('#players')!.innerHTML = `<span><i style="background:#ff5a4f"></i>YOU · ${health} HP</span><span><i style="background:#30a9ff"></i>BOT-01 · ${botHealth} HP</span>${remotePlayers.map(peer => `<span><i style="background:#a879ff"></i>${escapeHtml(peer.name)} · ${peer.health ?? 100} HP</span>`).join('')}`
